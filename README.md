...
After completing the training process (Cell 5), the notebook automatically saves the best-performing model checkpoint (`best_model_densenet121.pth`). Before running inference in **Cell 6**, update the checkpoint loading path to point to the saved model:
...
4. **Full-Fit Training (Cell 5):** Executes the training loop for 6 epochs using all development images. Automatic Mixed Precision (AMP) with `GradScaler` is used to maintain throughput. The best weights are saved automatically.
